...
print(list(players.copy().values())[1])


print(list(players.values()))
print(list(players.copy().values()))

teams_groupings = teams.items() # topls
print(teams_groupings)
print(len(teams_groupings))

print(list(teams_groupings)[1][1][0]) #grapped nested [team key] ...[player list]... [index position zero in the the players list]
...
# # player_names = list(players.copy().values()) - using the copy function correa Altuve Bregan Gattis Springer, accessed only by us #nested items now below
# teams = {
#   "astros" : ["Altuve", "Correa", "Bregman"],
#   "angels":  ["Trout", "Pujols"],
#   "yankees": ["Judge", "Stanton"],
